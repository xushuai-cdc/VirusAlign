    ...
    if query:
        with st.spinner("Matching..."):
            ...
            if result.is_matched():
                ...
                level_colors = {"Realm":"#E74C3C","Kingdom":"#2ECC71","Phylum":"#3498DB","Class":"#F39C12","Order":"#9B59B6","Family":"#1ABC9C","Genus":"#E91E63","Species":"#F1C40F"}
                for lvl in ["Realm","Kingdom","Phylum","Class","Order","Family","Genus","Species"]:
                    v = result.taxonomy.get(lvl, "-")
                    col = level_colors[lvl]
                    st.markdown(f'<span style="color:{col}"><b>{lvl}</b>: <i>{v}</i></span>', unsafe_allow_html=True)
   else:

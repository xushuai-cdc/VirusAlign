...
with st.sidebar:
    ...
    dm = engine._data
    st.markdown(f"🧬 **Species**: {dm.get_taxonomy_tree().get('species_count', 0):,} (ICTV)")
    st.markdown(f"🏷️ **Aliases**: {len(dm.get_alias_map()):,}")
    st.markdown(f"🔗 **NCBI Mapping**: {len(dm.get_ncbi_map()):,}")
    st.divider()
    st.markdown("**Session Stats**")
    stats = engine.match_stats
    st.markdown(f"🎯 Exact: {stats['exact']}  |  🔄 Alias: {stats['alias']}  |  🌐 API: {stats['ncbi_id']}  |  ❌ Unmatched: {stats['unmatched']}")
    st.divider()
    with st.expander("**Priority Pathogens (ICTV Verified)**", expanded=False):
        st.caption("Covering 17,554 species; focusing on high-risk groups:")
        pathogens = [

...
with st.sidebar:
    ...
    dm = engine._data
    sp_count = dm.get_taxonomy_tree().get("species_count", 0)
    st.markdown(f"🧬 {sp_count:,} 标准物种 (ICTV)")
    ...
    st.divider()
    st.markdown("**Priority Pathogens**")
    st.caption("System covers ALL 17,554 ICTV species. List below shows high-priority groups.")
    for cat, viruses in VIRUS_COVERAGE_CATEGORIES:
        st.markdown(f"- **{cat}**: {', '.join(viruses[:4])}")
    st.divider()
    st.caption("🔒 Privacy: Data processed in memory only; cleared after session.")
    ...
    if query:
        with st.spinner("Matching..."):
            ...
            if result.is_matched():
                ...
                enc = __import__("urllib.parse").quote(result.standard_name)
                st.markdown(
                    "🔗 **外部链接**: "
                ...
                with st.expander("各级分类"):
                    ...
                    color_map = {
                        "Realm": "#E74C3C", "Kingdom": "#E67E22", "Phylum": "#F1C40F",
                        "Class": "#2ECC71", "Order": "#3498DB", "Family": "#9B59B6",
                        "Genus": "#E91E63", "Species": "#1ABC9C"
                    }
                    html_rows = ""
                    for level, val in zip(levels, vals):
                        bg = color_map.get(level, "#95a5a6")
                        html_rows += f"<tr><td style='background:{bg};color:white;font-weight:bold;text-align:center;padding:6px 10px;border:1px solid #ddd'>{level}</td>"
                        if val != "-":
                            html_rows += f"<td style='color:{bg};font-style:italic;font-weight:bold;padding:6px 10px;border:1px solid #ddd'>{val}</td></tr>"
                        else:
                            html_rows += f"<td style='padding:6px 10px;border:1px solid #ddd;color:#999'>-</td></tr>"
                    st.markdown(
                        f"<table style='width:100%;border-collapse:collapse;margin-top:4px'>{html_rows}</table>",

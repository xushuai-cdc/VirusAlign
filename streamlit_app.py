...
st.markdown("""
    ...
    div.stButton > button {
        display: block;
        margin: 0 auto !important; /* 按钮在列内居中 */
        width: 100% !important;
        max-width: 160px !important; /* 核心：限制按钮最大宽度，防止拉得太长 */
        height: 2.8em !important;
        border-radius: 20px !important; /* 椭圆设计更像官网 */
        border: 1px solid #ccd1d9 !important;
        background-color: #f8f9fa !important;
...
with tab1:
    ...
    # 搜索布局
    _, search_col, _ = st.columns([2, 4, 2])
    with search_col:
        # 搜索框：使用 key 绑定
        ...
        st.markdown("<div style='text-align:center; font-size:1.1rem; color:#666; margin: 15px 0 10px 0'>Try Samples (点击直接体验):</div>", unsafe_allow_html=True)

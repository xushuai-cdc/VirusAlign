...
# ===== Tab 1: Quick Lookup =====
with tab1:
    st.write("输入病原体常用名、别名或 NCBI Taxonomy ID 进行对齐：")

    st.markdown("**Try samples:**")
    b = st.columns(5)
    sq = st.session_state
    if b[0].button("SFTSV"): sq["q"] = "SFTSV"
    if b[1].button("SARS-CoV-2"): sq["q"] = "SARS-CoV-2"
    if b[2].button("Dengue"): sq["q"] = "Dengue"
    if b[3].button("Zika"): sq["q"] = "Zika virus"
    if b[4].button("TaxID:3418604"): sq["q"] = "3418604"
...
st.caption("开发单位：中国疾病预防控制中心传染病预防控制所 ")
st.caption("项目资助：国家科技重大专项：X传染病全景病原图谱和序列数据库构建 (No. 2025ZD01900116)")
